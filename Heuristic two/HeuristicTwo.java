...
public class HeuristicTwo{
    ...
    public void getValues(){
        ...
        try {
            ...
            // System.out.println (this.mules);
            // System.out.println (this.numberOfDropBox);
            // System.out.println (this.numberOfNodes);
            // System.out.println (this.numberOfCenters);
            pathWayMatrix=new double[21][21];
            JSONArray pathWayList = (JSONArray) jsonObject.get("PathWayMatrix");
            for (int i = 0; i < 21; ++i) {
                JSONArray row = (JSONArray) pathWayList.get (i);
                for (int j = 0; j < row.size (); ++j) {
                    Object object = row.get(j);
                    if(object.getClass().getName().equals("java.lang.Long")) {
                        pathWayMatrix[i][j]= (long) object;
                    }
                    else if(object.getClass().getName().equals("java.lang.Double")) {
                        pathWayMatrix[i][j]= (double) object;
                    }
                }
                // System.out.println (" ");
            }
    ...
    public void displayTowers(int []towers) throws  IOException{
        ...
        try(FileWriter fileWriter = new FileWriter("InputSettings.txt")){
    ...
    public  void trajectory(int[] tower,int[] tempclusters) throws IOException
    {
        ...
        try(FileWriter fileWriter = new FileWriter("InputSettings.txt",true)){
            fileWriter.write(trajectory);
        }
        try (FileWriter file = new FileWriter("output3.json")) {
            file.write(obj.toJSONString());
        }
    }
